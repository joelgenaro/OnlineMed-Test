import React from 'react';
import { Card } from '@/components/ui/card';
import { Avatar, AvatarFallback } from '@/components/ui/avatar';
import { TestimonialData } from '@/types/medical';

interface TestimonialCardProps {
  testimonial: TestimonialData;
  showMoneyBackGuarantee?: boolean;
  mainTitle?: string;
  subtitle?: string;
  description?: string;
}

export const TestimonialCard: React.FC<TestimonialCardProps> = ({
  testimonial,
  showMoneyBackGuarantee = true,
  subtitle = "Work",
  description = "Answer a few questions, pay securely, and have your note delivered to your inbox in minutes."
}) => {
  return (
    <div className="h-full flex flex-col justify-between">
      <div className="space-y-6 pt-20">
        {showMoneyBackGuarantee && (
          <div className="flex items-center gap-2 text-medical-blue">
            <span className="h-5 w-5 inline-block" aria-label="badge-check">
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M16.6453 7.03281C16.3508 6.725 16.0461 6.40781 15.9312 6.12891C15.825 5.87344 15.8187 5.45 15.8125 5.03984C15.8008 4.27734 15.7883 3.41328 15.1875 2.8125C14.5867 2.21172 13.7227 2.19922 12.9602 2.1875C12.55 2.18125 12.1266 2.175 11.8711 2.06875C11.593 1.95391 11.275 1.64922 10.9672 1.35469C10.4281 0.836719 9.81563 0.25 9 0.25C8.18437 0.25 7.57266 0.836719 7.03281 1.35469C6.725 1.64922 6.40781 1.95391 6.12891 2.06875C5.875 2.175 5.45 2.18125 5.03984 2.1875C4.27734 2.19922 3.41328 2.21172 2.8125 2.8125C2.21172 3.41328 2.20312 4.27734 2.1875 5.03984C2.18125 5.45 2.175 5.87344 2.06875 6.12891C1.95391 6.40703 1.64922 6.725 1.35469 7.03281C0.836719 7.57188 0.25 8.18437 0.25 9C0.25 9.81563 0.836719 10.4273 1.35469 10.9672C1.64922 11.275 1.95391 11.5922 2.06875 11.8711C2.175 12.1266 2.18125 12.55 2.1875 12.9602C2.19922 13.7227 2.21172 14.5867 2.8125 15.1875C3.41328 15.7883 4.27734 15.8008 5.03984 15.8125C5.45 15.8187 5.87344 15.825 6.12891 15.9312C6.40703 16.0461 6.725 16.3508 7.03281 16.6453C7.57188 17.1633 8.18437 17.75 9 17.75C9.81563 17.75 10.4273 17.1633 10.9672 16.6453C11.275 16.3508 11.5922 16.0461 11.8711 15.9312C12.1266 15.825 12.55 15.8187 12.9602 15.8125C13.7227 15.8008 14.5867 15.7883 15.1875 15.1875C15.7883 14.5867 15.8008 13.7227 15.8125 12.9602C15.8187 12.55 15.825 12.1266 15.9312 11.8711C16.0461 11.593 16.3508 11.275 16.6453 10.9672C17.1633 10.4281 17.75 9.81563 17.75 9C17.75 8.18437 17.1633 7.57266 16.6453 7.03281ZM12.5672 7.56719L8.19219 11.9422C8.13414 12.0003 8.06521 12.0464 7.98934 12.0779C7.91346 12.1093 7.83213 12.1255 7.75 12.1255C7.66787 12.1255 7.58654 12.1093 7.51066 12.0779C7.43479 12.0464 7.36586 12.0003 7.30781 11.9422L5.43281 10.0672C5.31554 9.94991 5.24965 9.79085 5.24965 9.625C5.24965 9.45915 5.31554 9.30009 5.43281 9.18281C5.55009 9.06554 5.70915 8.99965 5.875 8.99965C6.04085 8.99965 6.19991 9.06554 6.31719 9.18281L7.75 10.6164L11.6828 6.68281C11.7409 6.62474 11.8098 6.57868 11.8857 6.54725C11.9616 6.51583 12.0429 6.49965 12.125 6.49965C12.2071 6.49965 12.2884 6.51583 12.3643 6.54725C12.4402 6.57868 12.5091 6.62474 12.5672 6.68281C12.6253 6.74088 12.6713 6.80982 12.7027 6.88569C12.7342 6.96156 12.7503 7.04288 12.7503 7.125C12.7503 7.20712 12.7342 7.28844 12.7027 7.36431C12.6713 7.44018 12.6253 7.50912 12.5672 7.56719Z" fill="#2563EB" />
              </svg>
            </span>
            <span className="text-sm font-medium">100% Money Back Guarantee</span>
          </div>
        )}

        <div>
          <h1 className="text-3xl font-bold text-medical-text leading-tight">
            Your <span className="text-medical-blue">{subtitle}</span> Note is
          </h1>
          <h1 className="text-3xl font-bold text-medical-text leading-tight">
            Minutes Away
          </h1>
        </div>

        <p className="text-medical-text-light text-sm leading-relaxed">
          {description}
        </p>
      </div>

      <div className='space-y-6'>
        <Card className="bg-background p-6 space-y-4 shadow-sm border-0">
          <div className="flex items-center gap-3">
            <Avatar className="h-12 w-12" style={{ backgroundColor: testimonial.avatarColor || 'hsl(var(--warning-amber))' }}>
              <AvatarFallback
                className="text-background font-semibold"
                style={{ backgroundColor: testimonial.avatarColor || 'hsl(var(--warning-amber))' }}
              >
                {testimonial.initials}
              </AvatarFallback>
            </Avatar>
            <div>
              <h3 className="font-semibold text-medical-text">{testimonial.name}</h3>
              <p className="text-sm text-medical-text-light">{testimonial.role} • {testimonial.location}</p>
            </div>
          </div>

          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              {[...Array(5)].map((_, i) => (
                <span key={i} className={`h-4 w-4 inline-block ${i < testimonial.rating ? '' : 'opacity-30'}`} aria-label="star">
                  <svg width="19" height="18" viewBox="0 0 19 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M9.65625 12.9911L13.567 12L15.2009 17.0357L9.65625 12.9911ZM18.6562 6.48214H11.7723L9.65625 0L7.54018 6.48214H0.65625L6.22768 10.5L4.1116 16.9821L9.68304 12.9643L13.1116 10.5L18.6562 6.48214Z" fill={i < testimonial.rating ? "#219653" : "#E5E7EB"} />
                  </svg>
                </span>
              ))}
            </div>
            <span className="text-sm text-medical-text-light">{testimonial.timeAgo}</span>
          </div>

          <p className="text-sm text-medical-text leading-relaxed">
            {testimonial.review}
          </p>
        </Card>

        <div className="flex justify-start gap-2">
          <div className="w-2 h-2 rounded-full bg-medical-blue"></div>
          <div className="w-2 h-2 rounded-full bg-medical-text-light opacity-30"></div>
          <div className="w-2 h-2 rounded-full bg-medical-text-light opacity-30"></div>
        </div>
      </div>
    </div>
  );
};